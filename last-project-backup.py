...
def ReadExcel():
    ...
    dictionary = { 'titles_from_xl_file':titles_from_xl_file, 'ratings_from_xl_file': ratings_from_xl_file, 'mod_date_from_xl_file': mod_date_from_xl_file }
    return dictionary
...
def WriteToExcel(dictionary):
    ...
    if len(titles_from_xl_file)>2:
        for i, title in enumerate(titles_from_xl_file):
            if ratings_from_xl_file[i] != ratings[i]:
